...
function showMacList(deviceAttr) {
    ...
    for (var i = 0; i < deviceAttr.length; i++) {
        ...
        html += '<tr><td>' + deviceAttr[i].id + '</td><td>' + temp + '</td><td>   <input type="button" id=' + buttonidName + ' class="button" value="'+get_lan_mac('Set')+'"></input> </td></tr>';
    }

    html += '<tr><td></td><td>' + '<input type="submit" id="Submit1" class="button" value="'+get_lan_mac('Save All')+'" lang="Save All"></input>' + '</td></tr>';
    $("#list").show();
    $("#table").append(html);
}
...
function showMacList2(deviceAttr) {
    ...
    for (var i = 0; i < deviceAttr.length; i++) {
        buttonidName = buttonIds[i];
        temp = '<input id="' + 'Macinput' + deviceAttr[i].id + '"' + ' name="wifi.mac" maxlength="17" value="' + deviceAttr[i].macaddr + '">';
        html += '<tr><td>' + deviceAttr[i].id + '</td><td>' + temp + '</td><td>   <input type="button" id=' + buttonidName + ' class="button" value="'+get_lan_mac('pass')+ '" ></input> </td></tr>';
    }

    html += '<tr><td></td><td>' + '<input type="submit" id="Submit1" class="button" value="Save all" lang="Save all"></input>' + '</td></tr>';
    $("#list").show();
    $("#table").append(html);
}
